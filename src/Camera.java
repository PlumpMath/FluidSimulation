...
public class Camera {
	private float screenX, screenY, previousX, previousY;
	...
	public Camera(Player player, Body boundary, String background)
	{
		...
		try {
			this.background = TextureLoader.getTexture("PNG", ResourceLoader.getResourceAsStream(background));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void update()
	{
		...
		*/
		...
		Vec2[] vertices = ((PolygonShape)player.getPlayerFixture().getShape()).m_vertices;
		Vec2 lower = vertices[2];
		Vec2 upper = vertices[0];
		float bodyWidth = (upper.x - lower.x) * Main.OPENGL_SCALE;
		float bodyHeight = (upper.y - lower.y) * Main.OPENGL_SCALE;
		previousX = screenX;
		previousY = screenY;
